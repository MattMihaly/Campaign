...
// Connection string. Accept either DATABASE_URL (generic) or POSTGRES_URI
// (the variable Northflank's managed Postgres addon provides), so linking the
// addon works with no extra mapping.
const DATABASE_URL = process.env.DATABASE_URL || process.env.POSTGRES_URI || '';
let pool = null;
let ready = false;
let lastInitError = null;

/* Diagnostic: report which DB-related env vars are visible (names only —
 * never values, which contain the password). */
function envDiagnostics() {
  const names = Object.keys(process.env).filter((k) =>
    /DATABASE|POSTGRES|^PG|^NF_/i.test(k)
  );
  return names.length ? names : ['(none found)'];
}
...
async function init() {
  console.log('DB env vars visible to the app:', envDiagnostics().join(', '));
  if (!DATABASE_URL) {
    lastInitError = 'no DATABASE_URL or POSTGRES_URI variable present';
    console.log('No DATABASE_URL / POSTGRES_URI set — incident archiving disabled (counter still works).');
    return false;
  }
  try {
    pool = new Pool({
    ...
    lastInitError = 'connection string present but connect failed: ' + e.message;
...
function initError() { return lastInitError; }
function hasUri() { return Boolean(DATABASE_URL); }
...
module.exports = { init, isReady, initError, hasUri, recordBatch, markCleared, stats };
