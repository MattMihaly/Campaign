...
try {
  ...
  db = {
    ...
    isReady: () => false,
    hasUri: () => false,
    initError: () => 'db module failed to load',
    recordBatch: async () => {},
    markCleared: async () => {},
...
const server = http.createServer((req, res) => {
  ...
  if (url === '/healthz') {
    ...
    res.end(JSON.stringify({
      status: 'ok', lastPoll, tracked: seen.size,
      archiving: db.isReady(),
      dbUriDetected: db.hasUri ? db.hasUri() : false,
      dbError: db.initError ? db.initError() : null,
    }));
    return;
  }
  if (url === '/api/tally') {
